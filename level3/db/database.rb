...
class Database
  ...
  def load_shifts
    shifts = []
    @data["shifts"].each do |data_shift|
      unless data_shift["user_id"].nil? # Escaping data where user_id is null
        shifts << Shift.new(id: data_shift["id"],
          worker: @workers[data_shift["user_id"].to_i - 1],
          planning: @planning,
          start_date: data_shift["start_date"])
      end
    end
    @planning.shifts = shifts
  end
end
